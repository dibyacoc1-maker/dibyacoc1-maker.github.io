...
async function saveShare(meta) {
  ...
  // Never store blob data in the meta node
  delete shareMeta.primaryFile;
  delete shareMeta.url;
...
async function loadShare(sid) {
  ...
  // primaryRef may be missing in older shares — default to index 0
  const primaryIdx = (meta.primaryRef !== undefined && meta.primaryRef !== null)
    ? meta.primaryRef : 0;
  const primaryFile = files[primaryIdx] || files[0] || null;

  return { ...meta, files, primaryFile };
}
